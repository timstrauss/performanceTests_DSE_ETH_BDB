    ...
    private fun executeTests(workerThreads: Int, time: Long, uuids: List<String>, con: BigchainDBConnectionDetails) {
        val setThreads = Array(workerThreads) {
            val thread = if (TestInfo.sameResource) {
                SetBoolThread(time, it, workerThreads, uuids[0], con)
            } else {
                SetBoolThread(time, it, workerThreads, uuids[it], con)
            }
            thread.start()
            thread
        }
        ...
        val getThreads = Array(workerThreads) {
            val thread = if (TestInfo.sameResource) {
                GetBoolThread(time, it, workerThreads, uuids[0], con)
            } else {
                GetBoolThread(time, it, workerThreads, uuids[it], con)
            }
            thread.start()
            thread
        }
